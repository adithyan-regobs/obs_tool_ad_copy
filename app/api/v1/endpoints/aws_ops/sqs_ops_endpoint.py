...
from app.services.aws_ops.sqs_ops import SQSOps

router = APIRouter()


class ValidateDuplicateQueueRequest(BaseModel):
    ...
    queue_name: str = Field(..., description="SQS queue name to validate")


@router.post(
    "/validate-duplicate-queue",
    response_model=ValidationResult,
    summary="Validate if an SQS queue name already exists",
)
async def validate_duplicate_queue(
    payload: ValidateDuplicateQueueRequest,
    user_and_tenant: Tuple[UserMstModel, TenantsMstModel] = Depends(get_current_user_and_tenant),
    db: AsyncSession = Depends(get_db),
) -> ValidationResult:
    """
    Check whether an SQS queue with the given name already exists for the
    ...
    ops = SQSOps(infrastructure_repo=repo)
    return await ops.duplicate_queue_validator(
