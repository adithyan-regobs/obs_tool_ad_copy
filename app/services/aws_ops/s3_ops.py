...
class S3Ops:
    ...
    async def duplicate_bucket_validator(
        self,
        tenant_code: str,
        product_code: str,
        ...
        """Check whether an S3 bucket with the given name already exists
        ...
        """
        # check_s3_bucket_exists uses first-match semantics, so pre-existing
        ...
        existing = await self.infrastructure_repo.check_s3_bucket_exists(
            bucket_identifier=bucket_name,
        ...
        if existing:
            return ValidationResult(
                ...
                description=(
                    f"Bucket '{bucket_name}' already exists for product "
        ...
        return ValidationResult(
            type="duplicateValidation",
            description=f"Bucket '{bucket_name}' is available.",
            valid=True,
        )
