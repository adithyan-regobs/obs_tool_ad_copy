...
class CanvasGeoLocation(BaseModel):
    ...
    geoLocCode: Optional[str] = Field(default=None, description="Matched geo_loc_mst.code for this geo location")
...
class CanvasEksCluster(BaseModel):
    ...
    infrastructureMstCode: Optional[str] = Field(default=None, description="infrastructure_mst.code for this cluster")
    infrastructureMstName: Optional[str] = Field(default=None, description="infrastructure_mst.name for this cluster")
    infraStatus: Optional[str] = Field(default=None, description="infrastructure_mst.infra_status for this cluster")
    isRegistered: bool = Field(default=False, description="Whether this cluster is registered in infrastructure_mst (service creation allowed)")
...
class CanvasNodeVariableItem(BaseModel):
    """Canvas node variable metadata — value fetched on-demand from AWS"""
    id: int
    code: str
    name: str
    type: str = "secret"
    secret_arn: Optional[str] = None
    referenced_variable_id: Optional[int] = None


class CanvasVariableRefItem(BaseModel):
    """Canvas variable reference — points to a source variable by DB id"""
    dbId: int = Field(..., description="This ref row's own variable_mst.id — used for deletion")
    name: str = Field(..., description="Alias name shown on this node (e.g. POSTGRES_HOST)")
    refVariableId: int = Field(..., description="DB id of the source variable being referenced")


class CanvasPlacementContext(BaseModel):
    """Minimal canvas placement context — geoLocations/accounts/cloudRegions only.

    Used when a caller needs just enough data to resolve a single resource's
    placement (account + cloud region for a chosen geo_loc), without paying the
    cost of fetching the full VPC/resource tree.
    """
...
class CanvasApiResponse(BaseModel):
    ...
    Node variables and variable refs are served separately by
    `GET /canvas-variables` so post-deploy refreshes don't force a full canvas
    rebuild (which would clobber any open resource-detail form state).
    """
    geoLocations: List[CanvasGeoLocation] = Field(default=[], description="Geographic locations")
    accounts: List[CanvasAccount] = Field(default=[], description="Cloud provider accounts")
    cloudRegions: List[CanvasCloudRegion] = Field(default=[], description="Cloud regions")
    ...
    showEmptyContainers: bool = Field(default=False, description="True when serving placeholder data for new tenants")
    forPaas: bool = Field(default=False, description="True when tenant uses PaaS placeholder data")


class CanvasVariablesResponse(BaseModel):
    """Response for `GET /canvas-variables`.

    Both maps are keyed by `transaction_code` (= canvas node
    `settings.resourceDbCode`). The frontend remaps the outer key to the
    canvas `node.id` using its current nodes state, so the backend never
    needs to know the canvas layout to serve this endpoint.
    """
    nodeVariables: Dict[str, List[CanvasNodeVariableItem]] = Field(
        default_factory=dict,
        description="Per-resource variables, keyed by resourceDbCode",
    )
    variableRefs: Dict[str, List[CanvasVariableRefItem]] = Field(
        default_factory=dict,
        description="Per-resource variable references, keyed by target resourceDbCode",
    )
