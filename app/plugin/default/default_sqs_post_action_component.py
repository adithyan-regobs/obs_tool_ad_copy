"""
Default SQS Post-Action Component

Runs after the SQS script gen component has staged the terragrunt.hcl.
Saves the queue URL to variable_mst as a VARIABLE so EKS deployments can
reference it via SQS_QUEUE_URL.

The full AWS queue name is pre-computed and stored in infra.locator['queue_name']
by make_infrastructure_mst_sqs() at record creation time.
...
class DefaultSqsPostActionComponent:
    ...
    async def run(
        ...
        if case == "create_queue":
            ...
            self.logger.warning(
                "[SQSPostAction] Unknown case_ref_code=%r — skipping", case
            )
    ...
    async def _handle_create(
        ...
        if not all([db, infrastructure_mst_code, environment]):
            self.logger.warning(
                "[SQSPostAction] Missing required fields — skipping. "
        ...
        if not infra:
            self.logger.warning(
                "[SQSPostAction] Infrastructure record not found: %s", infrastructure_mst_code
            )
            return

        queue_name = (infra.locator or {}).get("queue_name", "")
        if not queue_name:
            self.logger.warning(
                "[SQSPostAction] locator.queue_name missing for infra_code=%s", infrastructure_mst_code
            )
            return
    ...
    async def _save_variable(
        ...
        try:
            ...
            self.logger.warning(f"[SQSPostAction] Unknown environment '{environment}' — skipping")
            return
        ...
        results = await SecretConfigClient().upsert_variables(
            ...
            items=[{
                ...
                "description": f"{key} for SQS queue {infrastructure_mst_code}",
                "metadata_json": {"application_code": application_code},
            }],
        )
        if results and results[0]["status"] == "unchanged":
            self.logger.info(
                "[SQSPostAction] %s unchanged for infra_code=%s env=%s — skipping",
                key, infrastructure_mst_code, environment,
            )
            return

        self.logger.info(
            "[SQSPostAction] Saved %s=%s for infra_code=%s env=%s",
