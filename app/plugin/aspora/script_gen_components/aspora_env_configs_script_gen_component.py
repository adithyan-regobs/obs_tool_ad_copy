"""
Aspora Env Configs Script Generation Component

Creates configs.json for ECS services when missing.
"""
...
class AsporaEnvConfigsScriptGenComponent:
    """Component for generating env configs.json files."""
    ...
    async def generate(
        ...
        product_name = (
            ...
            or config_snapshot.get("application_name")
        ...
        if not service_name:
            raise ValueError("service_name is required for env configs generation")
        ...
        if skip_env_files:
            logger.warning(
                "Skipping env configs creation for Falcon service: product=%s, service=%s, tenant=%s",
        ...
        if existing_file.get("exists"):
            ...
            with log_timing(self.logger, f"{component_name}.script_generation", context=gen_context):
                content = json.dumps({"dummy_config": "dummy-value"}, indent=2) + "\n"
