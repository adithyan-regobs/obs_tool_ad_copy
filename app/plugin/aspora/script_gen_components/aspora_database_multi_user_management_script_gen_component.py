...
class AsporaDatabaseMultiUserManagementScriptGenComponent:
    """
    ...
    async def generate(
        ...
        # New format: users are inside the server config, each with their own grants
        ...
        # Iterate over all users in the server config and apply each one sequentially
        preview_hcl = ""
        for user_entry in users_list:
            ...
            gen_context = f"path={file_path} user={username} mode=update"
            with log_timing(self.logger, f"{component_name}.script_generation", context=gen_context):
                if db_type == 'mysql':
                    existing_content = self._generate_mysql_user(
                ...
                user_preview = self._preview_db_user_hcl(
                    db_type=db_type,
                    username=username,
                    password=password,
                    grants=grants
