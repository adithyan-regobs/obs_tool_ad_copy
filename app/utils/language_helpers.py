...
def is_go_language(language_name: Optional[str]) -> bool:
    """
    Check if language is a Go variant.

    Handles: "Go 1.23", "Go 1.24", "Go 1.25", etc.
    Based on language_ref entries: GO_1_23, GO_1_24, GO_1_25

    Args:
        language_name: Language name from language_ref.name

    Returns:
        True if Go, False otherwise
    """
    if not language_name:
        return False
    first_word = language_name.lower().strip().split()[0]
    return first_word == "go"


def is_python_language(language_name: Optional[str]) -> bool:
    """
    Check if language is a Python variant.

    Handles: "Python 3.9", "Python 3.10", "Python 3.11", etc.
    Based on language_ref entries: PYTHON_3_9, PYTHON_3_10, PYTHON_3_11, etc.

    Args:
        language_name: Language name from language_ref.name

    Returns:
        True if Python, False otherwise
    """
    if not language_name:
        return False
    first_word = language_name.lower().strip().split()[0]
    return first_word == "python"
...
def get_dockerfile_path(
    ...
    Rules:
    - Java with build_path (shared library/monorepo): docker/{service-name}/Dockerfile
    - Go: Always Dockerfile (root) - build_path not used for Go
    - Otherwise: Dockerfile (root)

    Args:
        service_name: Service name from services_mst.name
        build_path: Build path from service_config.config.build_path (Java monorepo only)
        is_java: Whether the language is Java

    Returns:
        Dockerfile path (e.g., "docker/my-service/Dockerfile" or "Dockerfile")
    """
    # Only Java uses build_path for docker path; Go always uses root
    if is_java and build_path:
